...
def get_role_for_security_level(security_level):
    return SECURITY_LEVEL_ROLES.get(int(security_level or 0))
...
def _authenticate_db_user(username, password, role):
    ...
    if db_role is None or db_role != role:
        return None
...
def authenticate_user(username, password, role):
    ...
    try:
        db_user = _authenticate_db_user(username, password, role)
