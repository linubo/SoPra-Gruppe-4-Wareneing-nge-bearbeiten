...
REQUIRED_VIEWS = [
    ...
    ("upd_views", "V_UPD_G04_PURCHASE_ORDER"),
...
VIEW_FALLBACKS = {
    ("lov_views", "LOV_STATUS_GOODS_RECEIPT"): [
        ("dbo", "LOV_STATUS_GOODS_RECEIVED_DE"),
        ("dbo", "LOV_STATUS_GOODS_RECEIVED"),
    ],
}


CODE_TYPE_FALLBACKS = {
    ("lov_views", "LOV_STATUS_GOODS_RECEIPT"): "GOODS_RECEIPT",
    ("lov_views", "LOV_GOODS_CONDITION"): "CONDITION_ID",
    ("lov_views", "LOV_STATUS_SUPPLIER_INVOICE"): "SUPPLIER_INVOICE",
}
...
def _code_type_exists(code_type):
    result = fetch_all("""
        SELECT TOP 1
            ID_CODE
        FROM dbo.T_CODE
...
def _view_status(schema, view_name):
    if _view_exists(schema, view_name):
        return True, ""

    for fallback_schema, fallback_name in VIEW_FALLBACKS.get((schema, view_name), []):
        if _view_exists(fallback_schema, fallback_name):
            return True, f"Fallback: {fallback_schema}.{fallback_name}"

    code_type = CODE_TYPE_FALLBACKS.get((schema, view_name))

    if code_type and _code_type_exists(code_type):
        return True, f"Fallback: dbo.T_CODE/{code_type}"
...
def db_status():
    ...
    if connection_status["connected"]:
        for schema, view_name in REQUIRED_VIEWS:
            try:
                exists, source = _view_status(schema, view_name)
