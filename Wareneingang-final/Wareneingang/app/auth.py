...
def is_logged_in():
    return bool(session.get("USERNAME") and session.get("SECURITYLEVEL"))
...
def current_role():
    ...
    if security_level >= ROLE_SECURITY_LEVELS[ADMIN_ROLE]:
        return ADMIN_ROLE

    return SECURITY_LEVEL_ROLES.get(security_level, session.get("ROLE"))


def current_security_level():
    security_level = session.get("SECURITYLEVEL")

    if security_level is not None:
        return int(security_level or 0)

    return ROLE_SECURITY_LEVELS.get(session.get("ROLE"), 0)


def is_admin():
    return current_security_level() >= ROLE_SECURITY_LEVELS[ADMIN_ROLE]
...
def require_roles(*roles):
    minimum_level = min(ROLE_SECURITY_LEVELS.get(role, 999) for role in roles)
...
def can_view_goods_receipts():
    return current_security_level() >= ROLE_SECURITY_LEVELS[LAGER_ROLE]
...
def can_change_goods_receipt_status(current_status, target_status):
    ...
    if security_level == ROLE_SECURITY_LEVELS[LAGER_ROLE]:
        return (current_status, target_status) in lager_transitions

    if security_level == ROLE_SECURITY_LEVELS[MANAGEMENT_ROLE]:
