...
def _fetch_all_goods_receipts_from_db():
    queries = [
        ...
        """
            SELECT
                gr.*,
                status_lov.GOODS_RECEIVED_STATUS AS STATUS_NAME
            FROM list_views.V_LIST_GOODS_RECEIPT gr
            LEFT JOIN dbo.LOV_STATUS_GOODS_RECEIVED_DE status_lov
                ON status_lov.CODE_ID = gr.STATUS
            ORDER BY gr.GOODS_RECEIPT_ID DESC
        """,
        """
            SELECT
...
def _fetch_items_by_goods_receipt_id_from_db(goods_receipt_id):
    queries = [
        ...
        """
            SELECT
                item.*,
                condition_lov.CONDITION_STATUS AS CONDITION_NAME
            FROM list_views.V_LIST_GOODS_RECEIPT_ITEM item
            LEFT JOIN dbo.LOV_GOODS_CONDITION condition_lov
                ON condition_lov.CODE_ID = item.CONDITION_ID
            WHERE item.GOODS_RECEIPT_ID = ?
            ORDER BY item.GOODS_RECEIPT_ITEM_ID
        """,
    ]
...
def _po_status_for_goods_receipt_status(target_status):
    if int(target_status) == 202:
        return PO_STATUS_FULLY_DELIVERED

    if int(target_status) == 205:
        return PO_STATUS_SENT_TO_SUPPLIER

    return None


def _update_purchase_order_status_with_cursor(cursor, po_id, po_status):
    updated_by = current_username() if has_request_context() else None
    updated_by = updated_by or "G04_WARENEINGANG"
    cursor.execute(
        "EXEC stored_proc.sp_update_po_status ?, ?, ?",
        po_id,
        po_status,
        updated_by
    )
...
def _update_goods_receipt_status_in_db(goods_receipt_id, target_status):
    ...
    if goods_receipt is None:
        raise ValueError("Wareneingang wurde nicht gefunden.")
    ...
    def transaction(cursor):
        ...
        po_status = _po_status_for_goods_receipt_status(target_status)

        if po_status is not None:
            _update_purchase_order_status_with_cursor(
                cursor,
                goods_receipt["PO_ID"],
                po_status
            )
...
def update_goods_receipt_status(goods_receipt_id, target_status):
    ...
    if target_status == 202:
        ...
        if purchase_order is not None:
            purchase_order["STATUS"] = PO_STATUS_FULLY_DELIVERED
    ...
    if target_status == 205:
        ...
        if purchase_order is not None:
            purchase_order["STATUS"] = PO_STATUS_SENT_TO_SUPPLIER
